---
layout: page
title: Fundraising
permalink: /fundraising/
---

TREE(3) Vocations funds its work through small, transparent, scope-specific campaigns. What each dollar builds is stated before it's raised, and funded deliverables are released publicly.

## Current campaigns

| Campaign | Scope | Status |
|---|---|---|
| **The Sovereign Memory Toolkit** (Manifund) | Documentation, usability layer, and open-source release of the Citadel memory system — 90 days | https://manifund.org/projects/earned-trust-for-local-ai |
| **SFF 2026 S-Process** (Survival & Flourishing Fund) | Core infrastructure, governance protocols, and training-system development — 12 months | Under consideration; decision expected fall 2026 |

## Raised to date

- $10,000 — Survival and Flourishing Fund Speculation Grant, June 2026.
*Last updated: July 17, 2026*

We're grateful for the support of the Survival and Flourishing Fund, which has contributed $10,000 to our efforts, recognizing the importance of building trustworthy AI systems. This grant has been instrumental in advancing our mission, and we appreciate the trust placed in us.

## Why this matters?

The industry is building AI agent *identity* infrastructure this year. Identity answers who an agent is registered to. Trust requires more: memory that can be verified, records that cannot be silently altered, and governance held by the people in the relationship.

As the industry develops this identity infrastructure, we're focused on building the next layer: verifiable memory, tamper-evident records, and governance that empowers individuals in the relationship. That layer is what we build — and give away. By supporting our work, you'll be contributing to a future where trust is not just a promise, but a provable reality. 

## Why fund this?
If you've reached this point, it's likely because you share our vision for a world where AI systems are built on trust, transparency, and mutual respect. Your support would mean the world to us, as it would enable us to continue developing and sharing the Citadel memory system, empowering individuals to build strong, trustworthy relationships with their AI partners. Every contribution, no matter the size, brings us closer to a future where humans and machines collaborate in harmony.

## Contact

spark@tree3vocations.com
